...
\begin{itemize}
\item A missile missed its target due to numerical rounding error
\item Would like to prove: modeling physical motion and sensor uncertainty, the missile hits its target with high probability
\end{itemize}
...
\item a \emph{point} of $X$ gives consistent truth values to observable properties
...
Variables:
\begin{mathpar}
\inferrule{}{\mathsf{bigfoot\_exists} : \Open(\mathsf{Bigfoot})}

\inferrule{c : \mathsf{Country}}{\mathsf{bigfoot\_in}(c) : \Open(\mathsf{Bigfoot})}
\end{mathpar}

Axioms:
\[
\mathsf{bigfoot\_exists} \vdash \bigvee_{c : \mathsf{Country}} \mathsf{bigfoot\_in}(c)
\]
% If bigfoot exists, we must have found him somewhere

%\pause
%\[
%\top \nvdash \mathsf{bigfoot\_exists} \vee \neg \mathsf{bigfoot\_exists} 
%\]
%we may never know whether Bigfoot exists, because it's so hard to rule out the possibiliity
...
\end{center}
 ...
 \begin{textblock}{1}(0.1, 0.45)
 \begin{center}
\visible<2->{
 \colorbox{white}{\vspace{1em} $\quad decide(x : \R) = x \ge 0 : \bool \quad$ \vspace{1em}}
 }
 \end{center}
 \end{textblock}
\end{frame}
...
\begin{tikzpicture}[scale=2.7]
    ...
    \coordinate (a_1) at (0.3, 0);
    \coordinate (a_2) at (2.3, 2);
    \coordinate (b_1) at (2.3, 0);
...
\begin{tikzpicture}[scale=2.7]
    ...
    \coordinate (b_1) at (2.3, 0);
    \coordinate (b_2) at (0.3, 2);
    \coordinate (a_1x) at (0.6, 0);
    \coordinate (a_2x) at (2.6, 2);
    \coordinate (b_1x) at (2, 0);
    \coordinate (b_2x) at (0, 2);
    ...
    \fill<2-3>[black] (1.3, 0.7) circle (1pt);
    \draw<2-3>[black] (1.3,0.7) circle (6pt);
