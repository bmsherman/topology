...
\end{cases},
\end{align*}
where for any space $A$, $\mathsf{abort} : \Zero \hookto A$ is the trivial embedding of the empty space $\Zero$. Patterns that the scrutinee falls into reduce to the embedding $\mathsf{id} : \One \hookto \One$ (the wildcard pattern), and patterns which it misses entirely reduce to $\mathsf{abort}$. We can then collect only the possible branches,
\begin{align*}
\mathsf{or}(\mathsf{up}(\mathsf{true}), \bot) = \mathsf{cases}(\mathsf{tt})
\begin{cases}
\wildcard &\Branch \up(\btrue)
