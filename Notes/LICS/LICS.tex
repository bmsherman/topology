...
\begin{align*}
\iimg{(f \circ x)}(U) &= \bigvee_{i : I} \iimg{(e_i \circ x_i \circ \tilde{q}_i^{-1})}(U)
...
We can use \grammar{this} to approximately query whether an arbitrary continuous map $f : \R \cto \R$ has no roots on an arbitrary closed interval, with the binary cover
\begin{align*}
\mathsf{no\_roots}_{\varepsilon} &: \{ (a, b) : \R \times \R \suchthat a < b\} \cto \BCover
\\ \mathsf{no\_roots}_\varepsilon(\ell) &\triangleq
  \forall_{\mathsf{HeineBorel}(\ell)}(\fun{x}{[ 0 < |f(x)| ]_\varepsilon}).
\end{align*}
