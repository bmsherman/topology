...
\item \emph{Gluing}: \note{People find this confusing} nondeterministic unions (maxima with respect to specialization order) exist for each pair of branches on their overlap.
\end{enumerate}
...
With this formulation, it simple to prove the cases cover $\Viet(\R)$, as they are the pullbacks along $\mathsf{no\_roots}_\varepsilon$ of the open embeddings $\mathsf{Left}$ and $\mathsf{Right}$, which together cover $\BCover$.
