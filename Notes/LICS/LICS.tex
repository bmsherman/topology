...
\end{align*}
Then binary covers form a De Morgan algebra with respect to this partial order. The top and bottom elements are $\mathsf{L}$ and $\mathsf{R}$, respectively. The corresponding conjunction, disjunction, and negation on $\BCover$ are continuous maps:
\begin{align*}
...
Here, we will describe some compact/overt subspaces and binary covers of $\R$ and use these to solve some approximate decision problems on $\R$ using the calculus of binary covers.
...
\begin{align*}
\mathsf{no\_roots}_{\varepsilon} &: \{ (a, b) : \R \times \R \suchthat a < b\} \cto \BCover
...
To prove covering, we first observe that  $(\fun{x}{|f(x)| < \varepsilon})$ and $(\fun{x}{f(x) \ne 0})$ form a binary cover of $\R$, as they are equivalent to the $\BCover$-valued continuous map
\[
\fun{x}{[0 < |f(x)|]_\varepsilon}
\]
(as in the $\mathsf{no\_roots}_\varepsilon$ example).
Since these opens form a binary cover, the covering condition holds by the same derivation that showed that the cases were covering in the definition of $\forall_{[\cdot]}f$.
