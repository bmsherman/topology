...
\]
As expected, $\iimg{f}$'s behavior is the union of the behaviors of all of the branches, which is exactly what the join represents. For $\iimg{f}$ to define a continuous map, one must confirm it preserves all joins and finite meets.
...
\]
then $f$, as defined by the inverse image map $\iimg{f}$ above, is a continuous map.
\end{theorem}
\begin{proof}
The map $\iimg{f}$ must preserve joins, since all the constituent maps $\iimg{e_i}$ do as well, and since joins preserve those properties. Thus, it remains to confirm that $\iimg{f}$ preserves finitary meets.
