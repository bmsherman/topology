...
CoFixpoint mapStream {A B} (f : A -> B) (s : Stream A) : Stream B :=
  match s with
  | Cons x xs => Cons (f x) (mapStream f xs)
  end.
...
CoFixpoint partialbind {A B} (m : Partial A) (f : A -> Partial B) : Partial B :=
  match m with
  | Now x => f x
  | Later m' => Later _ (partialbind m' f)
end.
...
CoFixpoint bernoulli (p : Dyad) : prob (Partial bool) := fun d => match p, d with
  Cons bp p', Cons bd d' => match bp, bd with
    | true , false => Now _ true
    | false, true  => Now _ false
    | _, _ => Later _ (bernoulli p' d')
    end
  end.
...
Definition go {A} (b : bool) (dist : prob A) : prob A := 
  fun d => dist (Cons b d).

(** The fundamental judgment we make about probability distributions.
    [Meas dist P c] means that the property [P] holds in _at least_ 
    the fraction [c] of the possible worlds of [dist]. [P] could be true
    even more often, but we have only provided _proofs_ in a fraction
    [c] of them. *)
Inductive Meas {A} (dist : prob A) (P : A -> Prop) : R -> Prop :=
  | MeasNone : Meas dist P 0
  | MeasUnit : (forall d, P (dist d)) -> Meas dist P 1
  | MeasCombine : forall {a b} (c : R),
       Meas (go true  dist) P a
     -> Meas (go false dist) P b
     -> (a + b)%R = (c * 2)%R
     -> Meas dist P c.


Definition pAnd {A} (P Q : A -> Prop) (x : A) : Prop := P x /\ Q x.
Definition pNeg {A} (P : A -> Prop) (x : A) : Prop := ~ (P x).

(** The fundamental judgment of conditional probability. To prove that
    the probability of [P] given [Q] is at least [ans], we must be able
    to do the following:
    - Refute the possibility of some fraction [denom] of the worlds
      assuming that [Q] holds; that is, prove [Meas dist (pNeg Q) denom].
    - Prove that both [P] and [Q] hold in some fraction [num] of the
      worlds. These worlds must be disjoint from those which refuted [Q]
      if our logic is consistent.
    - Show that [num = ans * (1 - denom)].

    The larger we can make either [num] or [denom], the larger we can
    make [ans]. In keeping with the spirit of _ex falso quodlibet_,
    if you can refute "almost all" worlds given [Q], then you can assert
    that the probability of [P] given [Q] is whatever you want. *)

Inductive CondMeas {A} (dist : prob A) (P Q : A -> Prop) : R -> Prop :=
  | Cond : forall {num denom ans}, 
           num = (ans * (1 - denom)%R)%R
         -> Meas dist (pAnd P Q) num 
         -> Meas dist (pNeg Q) denom
         -> CondMeas dist P Q ans.
...
Lemma probImpl {A} (dA : prob A) (P : A -> Prop)
  : forall {c}, Meas dA P c -> 
    forall {B} (dB : prob B) (Q : B -> Prop),
    (forall d, P (dA d) -> Q (dB d)) -> Meas dB Q c.
Proof.
intros c measA.
induction measA; intros.
- apply MeasNone.
- apply MeasUnit. auto.
- apply (@MeasCombine _ _ _ a b).
  + eapply IHmeasA1. intros d Pd. unfold go. unfold go in Pd.
    apply H0. assumption.
  + eapply IHmeasA2. intros d Pd. unfold go. unfold go in Pd.
    apply H0. assumption.
  + assumption. 
Qed.
